#!/usr/bin/python
"""Crossword grid extraction tool using computer vision.

This module provides functionality to extract, straighten, and digitize
crossword puzzle grids from images into binary matrices.
"""
...
def detect_grid_dimensions(
    ...
    if len(peaks_cols) > 1:
        ...
        # Adaptive refinement: Detect if we have a bimodal distribution (noise + real lines)
        # This happens when some grid lines are double-detected or noise is picked up
        if len(col_diffs) > 4:
            # Sort the spacings and look for a gap indicating two groups
            sorted_diffs = np.sort(col_diffs)
            # Check if there's a large gap between small and large spacings
            # Calculate gaps between consecutive sorted spacings
            spacing_gaps = np.diff(sorted_diffs)
            if len(spacing_gaps) > 0:
                max_gap_idx = np.argmax(spacing_gaps)
                max_gap = spacing_gaps[max_gap_idx]
                # If the largest gap is significant (>30% of median), we likely have two groups
                if max_gap > median_col_width * 0.3:
                    # Use the larger group (actual grid lines, not noise)
                    larger_spacings = sorted_diffs[max_gap_idx + 1:]
                    if len(larger_spacings) >= 3:  # Need at least a few samples
                        # Re-detect using the larger spacing as min_distance
    ...
    if len(peaks_rows) > 1:
        ...
        if len(row_diffs) > 4:
            ...
            if len(spacing_gaps) > 0:
                max_gap_idx = np.argmax(spacing_gaps)
                max_gap = spacing_gaps[max_gap_idx]
                if max_gap > median_row_height * 0.3:
                    larger_spacings = sorted_diffs[max_gap_idx + 1:]
...
class Application:
    """Crossword grid extraction application.

    This class provides a CLI interface for extracting crossword grids from images
    using computer vision techniques.

    Attributes:
        image_path: Path to the input image file
        image: Loaded OpenCV image array
    """

    def __init__(self, input: str, verbose: bool = False):
        """Initialize the application.

        Args:
            input: Path to input crossword image
        ...
        """
        logger.remove()
        logger.add(sys.stderr, level="TRACE" if verbose else "INFO")

        self.image_path = Path(input)

        # Validate input file
        if not self.image_path.exists():
            raise FileNotFoundError(f"Input file not found: {self.image_path}")

        if not self.image_path.is_file():
            raise ValueError(f"Input path is not a file: {self.image_path}")

        # Load and validate image
        self.image = cv2.imread(str(self.image_path))
        if self.image is None:
            raise ValueError(
                f"Failed to load image from {self.image_path}. "
                f"Ensure the file is a valid image format (JPEG, PNG, etc.)"
            )

        logger.info(f"Loaded image: {self.image_path} ({self.image.shape[1]}x{self.image.shape[0]})")

    def extract(self, output: str = "extracted_grid.jpg", visualize: bool = False):
        """Extract and straighten the crossword grid.

        Args:
            output: Path for output image file
            visualize: Show detected contour on original image

        Raises:
            GridExtractionError: If grid detection fails
        """
        try:
            warped, max_width, max_height = extract_grid(self.image)

            # Save the straightened grid image
            output_path = Path(output)
            cv2.imwrite(str(output_path), warped)
            logger.info(f"Saved straightened grid to: {output_path}")
            ...
            if visualize:
                ...
                self._visualize_extraction(vis_path)
    ...
    def size(self):
        """Detect and display grid dimensions (columns x rows).

        Raises:
            DimensionDetectionError: If dimension detection fails
        """
        try:
            cols, rows = detect_grid_dimensions(self.image)
    ...
    def convert_extracted(self):
        warped, max_width, max_height = extract_grid(self.image)
        cols, rows = detect_grid_dimensions(warped)
        print(cols, rows)

    def convert(
        self, output: str = "crossword_grid.csv", threshold: Optional[int] = None, visualize: bool = False
    ):
        """Full pipeline: extract grid, detect dimensions, and convert to binary matrix.

        Args:
            output: Path for output CSV file
            threshold: Manual intensity threshold (auto-detected if None)
            visualize: Save visualization images

        Raises:
            GridExtractionError: If grid extraction fails
            DimensionDetectionError: If dimension detection fails
        """
        try:
    ...
    def _visualize_extraction(self, output_path: Path):
